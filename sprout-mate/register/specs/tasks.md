...
- 保存 `code_cipher`，用于不保存明文的前提下重复生成时返回已有邀请码。
- 为 `code_hash` 添加唯一约束。
...
- 新增 `registerJoinGroup`：校验邀请码，创建 `USER` 用户，物理删除已消费邀请码。
...
- 创建分组允许同名 `group.name`，不做全局同名拦截。
- 账号密码注册的账号只能包含英文和数字。
- 邀请码注册后，用户 `role = USER`。
- 邀请码被成功使用后物理删除，不能再次使用。
- 当前分组已有未使用邀请码时，生成接口直接返回已有邀请码，不新增记录。
...
- 创建分组模式先输入分组名，再在当前页面区域展示注册方式选择。
- 创建分组注册方式支持微信注册和账号密码注册，提交时必须携带分组名。
- 加入分组模式输入邀请码。
- 提交时调用对应注册动作。
- 加入分组模式先输入邀请码，再在当前页面区域展示注册方式选择。
- 加入分组注册方式支持微信注册和账号密码注册，提交时必须携带邀请码。
- 账号输入框只能输入英文和数字；提交前也必须校验账号字符。
- 提供返回登录页入口。
...
- 展示成员头像、昵称、角色。
- 管理员可在弹框中选择成员角色，并通过底部「保存权限」按钮确认提交。
- 提供「生成邀请码」按钮。
...
- 角色选择项不展示 loading，也不在点击选项时直接调用接口；保存 loading 展示在确认按钮上。
